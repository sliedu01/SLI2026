    ...
    const metricAnalysis = [
      {
        ...
        criteria: "평가 기준: 70% 이상 (매우 높음) / 30%~70% 미만 (중간) / 30% 미만 (낮음)"
      },
      {
        ...
        criteria: "평가 기준: 0.8 이상 (큰 효과) / 0.5~0.8 미만 (중간 효과) / 0.2~0.5 미만 (작은 효과) / 0.2 미만 (효과 미미)"
      },
      {
        name: "만족도 및 추천 지수",
        value: `${sat}점`,
        interpretation: Number(sat) >= 4.5 ? "우수한 운영 품질 유지" : Number(sat) >= 4.0 ? "안정적인 운영 품질" : Number(sat) >= 3.5 ? "보통 수준의 운영" : "미흡 (개선 요망)",
        desc: "강사 전문성, 콘텐츠 적절성, 운영 인프라에 대한 학습자의 종합적인 체감 품질입니다.",
        criteria: "평가 기준: 4.5 이상 (최우수) / 4.0~4.5 미만 (우수) / 3.5~4.0 미만 (보통) / 3.5 미만 (미흡)"
      }
    ];
