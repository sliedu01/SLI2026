    ...
    const metricAnalysis = [
      {
        ...
        desc: "사전 지식 수준을 고려하여 순수하게 교육을 통해 성취한 성장의 비율을 의미합니다. [판단 기준: 70% 이상 매우 높은 향상, 30%~70% 미만 중간 향상, 30% 미만 낮은 향상]"
      },
      {
        name: "Cohen's d (효과 크기)",
        value: cohen,
        interpretation: Number(cohen) >= 0.8 ? "강력한 교육 임팩트 확인" : Number(cohen) >= 0.5 ? "중간 수준의 실질적 변화" : Number(cohen) >= 0.2 ? "작은 수준의 변화" : "제한적인 변화",
        desc: "우연에 의한 변화가 아닌, 교육 프로그램이 학습자에게 미친 실제 영향력의 강도를 수치화한 것입니다. [판단 기준: 0.8 이상 큰 효과, 0.5~0.8 미만 중간 효과, 0.2~0.5 미만 작은 효과, 0.2 미만 효과 미미]"
      },
      {
        name: "만족도",
        value: `${sat}점 (${Math.round((stats.satAvg / 5) * 100)}%)`,
        interpretation: Number(sat) >= 4.5 ? "매우 만족 수준의 최상 품질" : Number(sat) >= 4.0 ? "만족 수준의 우수한 품질" : Number(sat) >= 3.5 ? "보통 수준의 운영" : "미흡 (개선 요망)",
        desc: "교육 과정, 강사 전문성, 운영 인프라 등에 대한 종합적인 만족 수준입니다. [판단 기준: 4.5 이상 매우 만족, 4.0~4.5 미만 만족, 3.5~4.0 미만 보통, 3.5 미만 미흡]"
      }
    ];
