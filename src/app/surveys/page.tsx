...
export default function SurveysPage() {
  ...
  const metricGuides = {
    satAvg: {
      title: "만족도 점수 (Satisfaction Score)",
      formula: "만족도 = (전체 응답자의 평균 점수)",
      criteria: "4.5이상 매우우수 | 4.0이상 우수 | 3.5이상 보통 | 3.0이상 미흡 | 3.0미만 매우미흡",
      effect: "교육과정의 만족도를 나타내며, 교육생들의 학습 경험과 교육 품질을 평가하는 지표입니다."
    },
    gain: {
      title: "학습 효율 및 전이도 (Hake's Gain %)",
      formula: "G = (사후점수 - 사전점수) / (최대점수 - 사전점수) * 100",
      criteria: "70%이상 높은향상(High) | 30~70% 중간향상(Medium) | 30%미만 낮은향상(Low)",
      effect: "교육을 통한 학습자의 역량 변화를 측정하며, 교육의 실질적인 효과성을 평가하는 지표입니다."
    },
    cohensD: {
      title: "통계적 효과 크기 (Cohen's d)",
      formula: "d = (사후점수평균 - 사전점수평균) / 전체 표준편차",
      criteria: "0.8이상 큰효과(Large) | 0.5이상 중간효과(Medium) | 0.2이상 작은효과(Small)",
      effect: "역량 변화의 통계적 유의성을 나타내며, 교육의 실질적인 변화 정도를 평가하는 지표입니다."
    },
    pValue: {
      title: "통계적 유의확률 (p-value)",
      formula: "대응표본 t-검정(Paired t-test) 유의확률",
      criteria: "p < 0.05 유의한 차이 | p < 0.01 매우 유의한 차이",
      effect: "분석결과가 우연에 의한 것인지 확인하며, 교육의 효과가 통계적으로 유의한지 평가하는 지표입니다."
    }
  };
  ...
  const handlePasteProcess = async () => {
    ...
    const { bulkAddResponses } = useSurveyStore.getState();
    ...
    try {
      ...
      if (newResponses.length > 0) {
        await bulkAddResponses(newResponses);
        setPasteContent('');
        setIsPasteDialogOpen(false);
        alert('데이터 분석 및 등록이 완료되었습니다.');
      } else {
    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white/50 backdrop-blur-xl p-8 rounded-[2.5rem] border border-slate-100 shadow-2xl">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3"><ClipboardCheck className="size-8 text-blue-600" /> 전문가 성과 진단 및 성과 관리</h1>
          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest font-mono">Expert Analytics System</p>
        </div>
        <div className="flex bg-slate-100 p-1.5 rounded-2xl shadow-inner">
          {[{ id: 'templates', label: '설문 템플릿', icon: Settings2 }, { id: 'data', label: '데이터 분석 대시보드', icon: BarChart3 }].map(tab => (
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-1000">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
             <Card className="rounded-[2.5rem] border-none shadow-xl bg-gradient-to-br from-emerald-500 to-emerald-600 text-white p-8 space-y-4">
                <div className="flex justify-between items-center opacity-80"><span className="text-[11px] font-black uppercase tracking-widest">운영 만족도</span><PieChartIcon className="size-5" /></div>
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] font-bold opacity-80 leading-tight">
                    100점 만점 환산 시 {Math.round(overallStats.satAvg * 20)}점<br/>
                    {overallStats.satAvg >= 4.5 ? '매우 우수한' : '준수한'} 만족도 지표
                  </p>
                </div>
             </Card>
             <Card className="rounded-[2.5rem] border-none shadow-xl bg-gradient-to-br from-blue-500 to-blue-600 text-white p-8 space-y-4">
                <div className="flex justify-between items-center opacity-80"><span className="text-[11px] font-black uppercase tracking-widest">학습 효율 및 전이도</span><TrendingUp className="size-5" /></div>
                <div className="space-y-1">
                  <div className="text-4xl font-black">{Math.round(overallStats.hakeGain * 100)}%</div>
                  <p className="text-[11px] font-bold opacity-80 leading-tight">
                    Hake&apos;s Gain 성과 환산<br/>
                    {overallStats.hakeGain >= 0.7 ? '매우 높은' : overallStats.hakeGain >= 0.3 ? '평균적인' : '다소 낮은'} 향상 수치
                  </p>
                </div>
             </Card>
             <Card className="rounded-[2.5rem] border-none shadow-xl bg-gradient-to-br from-amber-500 to-amber-600 text-white p-8 space-y-4">
                <div className="flex justify-between items-center opacity-80"><span className="text-[11px] font-black uppercase tracking-widest">통계적 효과 크기</span><BarChart2 className="size-5" /></div>
                <div className="space-y-1">
                  <div className="text-4xl font-black">{overallStats.cohensD.toFixed(2)}</div>
                  <p className="text-[11px] font-bold opacity-80 leading-tight">
                    Cohen&apos;s d 효과 환산<br/>
                    {overallStats.cohensD >= 0.8 ? '매우 큰' : overallStats.cohensD >= 0.5 ? '중간 정도의' : '미미한'} 유의미한 변화
                  </p>
                </div>
             </Card>
             <Card className="rounded-[2.5rem] border-none shadow-xl bg-gradient-to-br from-purple-500 to-purple-600 text-white p-8 space-y-4">
                <div className="flex justify-between items-center opacity-80"><span className="text-[11px] font-black uppercase tracking-widest">통계적 유의확률</span><CheckCircle2 className="size-5" /></div>
                <div className="space-y-1">
                  <div className="text-4xl font-black">{overallStats.pValue < 0.001 ? '< .01' : overallStats.pValue.toFixed(2)}</div>
                  <p className="text-[11px] font-bold opacity-80 leading-tight">
                    Paired t-test 분석 신뢰도<br/>
                    {overallStats.pValue < 0.05 ? '통계적으로 유의한 신뢰수준' : '통계적 유의성 확인 필요'} 단계
                  </p>
                </div>
             </Card>
          </div>
          
          <Card className="rounded-[3rem] border-none shadow-2xl bg-white p-10 mt-10">
            <div className="flex flex-col md:flex-row gap-10">
              <div className="flex-1 space-y-4 text-center md:text-left">
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                  ...
                  <h3 className="text-xl font-black text-slate-900 tracking-tight">데이터 기반 전문가 통합 진단 및 성과 요약</h3>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
             <div className="lg:col-span-1 space-y-6">
                <Card className="rounded-[2rem] border-none shadow-xl bg-white sticky top-10">
                   <CardHeader><CardTitle className="text-lg font-black flex justify-between items-center">설문 템플릿 목록 <Plus className="size-4 cursor-pointer" onClick={() => addTemplate({ name: '새로운 만족도 평가', type: 'SATISFACTION', questions: createDefaultQuestions('SATISFACTION') })} /></CardTitle></CardHeader>
             ...
             <div className="lg:col-span-3">
                <Card className="rounded-[3rem] border-none shadow-2xl bg-white overflow-hidden">
                    <CardHeader className="p-10 border-b border-slate-50 flex items-center justify-between">
                       ...
                       <Button onClick={async () => { if(editingTemplate) { await updateTemplate(editingTemplate.id, { name: editingTemplate.name, questions: editingTemplate.questions }); alert('저장되었습니다.'); } }} className="rounded-xl h-12 px-8 bg-blue-600 text-white font-black">저장</Button>
                    </CardHeader>
                    <CardContent className="p-0 divide-y divide-slate-100">
                       ...
                       <div className="p-10 flex justify-center gap-4">
                         <Button onClick={() => handleAddQuestion('SCALE')} variant="outline" className="h-12 border-dashed border-2 px-8 rounded-xl font-black text-blue-600 hover:bg-blue-50 transition-all">+ 역량 질문 추가</Button>
                         <Button onClick={() => handleAddQuestion('TEXT')} variant="outline" className="h-12 border-dashed border-2 px-8 rounded-xl font-black text-slate-600 hover:bg-slate-50 transition-all">+ 텍스트 질문 추가</Button>
                       </div>
                    </CardContent>
                </Card>
             </div>
          ...
          <div className="space-y-8 animate-in slide-in-from-bottom-5">
              ...
              <div className="flex flex-wrap gap-3 px-2">
                ...
                {selectedProjectIds.length === 0 && (
                  <div className="text-xs font-bold text-slate-400 py-2">프로젝트를 선택하시면 실시간 성과 분석 데이터가 로드됩니다.</div>
                )}
              </div>
             ...
             <div className="flex flex-wrap items-end gap-6 bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl overflow-visible">
                  <div className="flex gap-4 min-w-[320px]">
                    <div className="flex-1 space-y-1"><label className="text-[10px] font-black text-slate-400">시작일 기준</label><Input type="date" value={dataDateRange.start} onChange={(e)=>setDataDateRange(p=>({...p, start:e.target.value}))} className="h-12 bg-slate-50 border-none rounded-xl font-bold" /></div>
                    <div className="flex-1 space-y-1"><label className="text-[10px] font-black text-slate-400">종료일 기준</label><Input type="date" value={dataDateRange.end} onChange={(e)=>setDataDateRange(p=>({...p, end:e.target.value}))} className="h-12 bg-slate-50 border-none rounded-xl font-bold" /></div>
                  </div>
                      ...
                      <table className="w-full border-collapse">
                        <thead>
                          <tr className="bg-slate-50/50 border-b border-slate-100 text-[10px] font-black text-slate-400 uppercase">
                            <th className="p-6 text-left sticky left-0 z-40 bg-white/95 backdrop-blur shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] border-r min-w-[300px]">프로그램 / 분석 테마</th>
                            ...
                            {satTextQuestions.map((q, idx) => (
                             ...
                             <th className="p-4 text-center bg-emerald-50 text-[10px] w-20 text-emerald-700 font-black">
                               <Tooltip>
                                 <TooltipTrigger className="cursor-help uppercase">운영 만족도</TooltipTrigger>
                                 <TooltipContent className="p-5 bg-slate-900 text-white rounded-[1.5rem] border-none shadow-2xl space-y-3 min-w-[280px]">
                                   ...
                                   <div className="space-y-2 text-[11px] opacity-90 leading-relaxed font-medium">
                                     ...
                                     <p><span className="text-white font-black">[수준]</span> {metricGuides.satAvg.criteria}</p>
                                     <p><span className="text-white font-black">[해석]</span> {metricGuides.satAvg.effect}</p>
                                   </div>
                                 </TooltipContent>
                               </Tooltip>
                             </th>
                               ...
                               <Tooltip>
                                 ...
                                 <TooltipContent className="p-5 bg-slate-900 text-white rounded-[1.5rem] border-none shadow-2xl space-y-3 min-w-[280px]">
                                   ...
                                   <div className="space-y-2 text-[11px] opacity-90 leading-relaxed font-medium">
                                     ...
                                     <p><span className="text-white font-black">[수준]</span> {metricGuides.gain.criteria}</p>
                                     <p><span className="text-white font-black">[해석]</span> {metricGuides.gain.effect}</p>
                                   </div>
                                 </TooltipContent>
                               </Tooltip>
                             </th>
                               ...
                               <Tooltip>
                                 ...
                                 <TooltipContent className="p-5 bg-slate-900 text-white rounded-[1.5rem] border-none shadow-2xl space-y-3 min-w-[280px]">
                                   ...
                                   <div className="space-y-2 text-[11px] opacity-90 leading-relaxed font-medium">
                                     ...
                                     <p><span className="text-white font-black">[수준]</span> {metricGuides.cohensD.criteria}</p>
                                     <p><span className="text-white font-black">[해석]</span> {metricGuides.cohensD.effect}</p>
                                   </div>
                                 </TooltipContent>
                               </Tooltip>
                             </th>
                               ...
                               <Tooltip>
                                 ...
                                 <TooltipContent className="p-5 bg-slate-900 text-white rounded-[1.5rem] border-none shadow-2xl space-y-3 min-w-[280px]">
                                   ...
                                   <div className="space-y-2 text-[11px] opacity-90 leading-relaxed font-medium">
                                     ...
                                     <p><span className="text-white font-black">[수준]</span> {metricGuides.pValue.criteria}</p>
                                     <p><span className="text-white font-black">[해석]</span> {metricGuides.pValue.effect}</p>
                                   </div>
                                 </TooltipContent>
                               </Tooltip>
                             </th>
                                      ...
                                      <td className="p-4 sticky left-0 z-20 bg-inherit group-hover:bg-inherit transition-colors border-r min-w-[300px]">
                                        <div className="flex items-center gap-3" style={{ paddingLeft: `${depth * 1.5}rem` }}>
                                          ...
                                          <div className="flex flex-col gap-0.5">
                                            ...
                                            <span className="text-[11px] font-black text-slate-900 truncate max-w-[200px]">
                                              {p.name}
                                            </span>
                                            {p.level >= 3 && p.quota > 0 && (
                                              ...
                                              <Tooltip>
                                                ...
                                                <TooltipContent className="p-3 bg-white border-slate-100 shadow-xl rounded-xl">
                                                  <div className="text-[10px] space-y-1">
                                                    <p className="font-black text-blue-600 underline underline-offset-2">역량 향상도 산출 기초</p>
                                                    ...
                                                    <p className="text-slate-700 font-black pt-1 border-t border-slate-100 italic">
                                                      {`데이터 분석 결과 사전 대비 사후 역량이 약 ${Math.round(stats?.questionStats?.[i]?.impRate)}% ${stats?.questionStats?.[i]?.impRate >= 0 ? "향상" : "감소"}된 것으로 확인되었습니다.`}
                                                    </p>
                                                  </div>
                                                </TooltipContent>
                                              </Tooltip>
                                              ...
                                              <TooltipContent className="p-4 bg-white border-slate-100 shadow-2xl rounded-2xl">
                                                <div className="text-[11px] space-y-1.5">
                                                  <div className="flex items-center gap-1.5">
                                                    ...
                                                    <p className="font-black text-slate-800">프로그램 역량 향상률 (Skill Growth)</p>
                                                  </div>
                                                  ...
                                                  <p className="text-slate-700 font-black border-t border-slate-100 pt-1.5">
                                                     {`본 교육 과정을 통해 사전 대비 사후 역량이 약 ${Math.round(stats?.impRate)}% ${stats?.impRate >= 0 ? "성장" : "정체"}하였으며, 이는 ${stats?.impRate >= 10 ? "통계적으로 유의미한 성과" : "점진적인 성과 지표"}로 분석됩니다.`}
                                                  </p>
                                                </div>
                                              </TooltipContent>
                                            </Tooltip>
                                              ...
                                              <TooltipContent className="p-4 bg-white border-slate-100 shadow-2xl rounded-2xl">
                                                <div className="text-[11px] space-y-1.5">
                                                  <div className="flex items-center gap-1.5">
                                                    ...
                                                    <p className="font-black text-slate-800">Hake&apos;s Gain 학습 전이도 (Effectiveness %)</p>
                                                  </div>
                                            ...
                                            <TooltipContent className="p-6 bg-slate-900 border-none shadow-3xl rounded-[2rem] w-80 text-white">
                                              <div className="space-y-4">
                                                <div className="flex items-center gap-3 border-b border-white/10 pb-3">
                                                  ...
                                                  <div><p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest leading-none mb-1">Hake&apos;s Gain %</p><p className="text-white font-black">학습 효율 및 전이도</p></div>
                                                </div>
                                            ...
                                            <TooltipContent className="p-6 bg-slate-900 border-none shadow-3xl rounded-[2rem] w-80 text-white">
                                              <div className="space-y-4">
                                                <div className="flex items-center gap-3 border-b border-white/10 pb-3">
                                                  ...
                                                  <div><p className="text-[10px] font-black text-blue-400 uppercase tracking-widest leading-none mb-1">Cohen&apos;s d</p><p className="text-white font-black">통계적 효과 크기 (Effect Size)</p></div>
                                                </div>
                                            ...
                                            <TooltipContent className="p-6 bg-slate-900 border-none shadow-3xl rounded-[2rem] w-80 text-white">
                                              <div className="space-y-4">
                                                <div className="flex items-center gap-3 border-b border-white/10 pb-3">
                                                  ...
                                                  <div><p className="text-[10px] font-black text-purple-400 uppercase tracking-widest leading-none mb-1">p-Value</p><p className="text-white font-black">통계적 유의확률 (t-test)</p></div>
                                                </div>
                              ...
                              <body>
                                ...
                                <table>
                                  <thead>
                                    <tr>
                                      <th>핵심 역량 성취도 (AVG)</th>
                                      <th>학습 효율 및 전이도 (GAIN)</th>
                                      <th>통계적 효과 크기 (COHEN'S D)</th>
                                      <th>통계적 유의확률 (P-VALUE)</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    <tr>
                                ...
                                <p style="font-size: 10px; color: #64748b; margin-top: 5px; text-align: left;">
                                  ※ 주석: 사전 역량이 만점(5.00)인 학습자의 개별 향상도는 산출 불가(N/A) 처리하였으며, 전체 향상도는 그룹 전체의 사전/사후 평균 차이를 바탕으로 산출함.
                                </p>
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 border-b-2 border-slate-100 pb-12">
                            ...
                            <div className="flex items-center gap-8 bg-slate-50 px-10 py-8 rounded-3xl border border-slate-100 print:py-4 print:px-6">
                                <div className="text-right">
                                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">학습 효율 및 전이도 (GAIN)</p>
                                    <p className="text-4xl font-black text-blue-600 print:text-2xl">{Math.round((overallStats?.hakeGain || 0) * 100)}%</p>
                                </div>
                                <div className="h-12 w-px bg-slate-200" />
                                <div className="text-right">
                                    <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">핵심 역량 성취도 (AVG)</p>
                                ...
                                <table className="w-full text-left border-collapse">
                                  <thead className="bg-slate-50">
                                    <tr className="font-black text-slate-500 uppercase">
                                      ...
                                      <th className="p-3 text-center border-r">만족도 평균</th>
                                      <th className="p-3 text-center border-r">사전 역량</th>
                                      <th className="p-3 text-center border-r">사후 역량</th>
                                      <th className="p-3 text-center">역량 향상도</th>
                                    </tr>
                                  </thead>
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="rounded-2xl border border-slate-100 overflow-x-auto bg-white max-h-[500px] print:max-h-none shadow-sm relative">
                                <table className="w-full text-[9px] border-collapse min-w-[600px]">
                                  <thead className="bg-slate-50 sticky top-0 z-20">
                                    <tr className="font-black text-slate-500 border-b">
                                      ...
                                      <th className="p-3 border-r text-center">만족도 평균</th>
                                      <th className="p-3 border-r text-center">사전 역량</th>
                                      <th className="p-3 border-r text-center">사후 역량</th>
                                      <th className="p-3 text-center">역량 향상도</th>
                                    </tr>
                                  </thead>
                                  <tbody className="divide-y divide-slate-50">
                                    {(() => {
                                      ...
                                      const allRowData = selectedProjectIds.flatMap(pid => {
                                        ...
                                        return uniqueRids.map(rid => {
                                          ...
                                          return { rid, pid, sAvg, cPre, cPost, rGain };
                                        });
                                        ...
                                        <>
                                          {allRowData.map((row, rIdx) => (
                                            <tr key={`${row.pid}-${row.rid}-${rIdx}`} className="text-slate-600 group hover:bg-slate-50 transition-colors">
                                              ...
                                              <td className="p-3 text-center text-blue-700 font-bold">{row.cPre >= 5 ? 'N/A' : `${Math.round(row.rGain * 100)}%`}</td>
                                            </tr>
                                          ))}
                               ...
                               <div className="grid grid-cols-4 gap-4 mt-6">
                                 ...
                                 <div className="bg-white rounded-2xl border-2 border-slate-100 p-5 flex flex-col gap-2 shadow-sm">
                                    <p className="text-[10px] font-black text-emerald-600 uppercase tracking-tighter flex items-center gap-1.5">
                                       <TrendingUp className="size-3" /> 학습 효율 및 전이도 (GAIN)
                                    </p>
                                    <div className="flex items-baseline gap-1">
        ...
        <DialogContent className="max-w-4xl rounded-[3rem] p-12 bg-white shadow-3xl">
           ...
           <DialogFooter className="pt-8">
             <Button 
               ...
               className="bg-blue-600 text-white px-12 rounded-2xl font-black gap-2 transition-all"
             >
               {isProcessing ? (
                 <>
